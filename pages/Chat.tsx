    ...
    <div className="flex flex-col h-full relative">
      ...
      <div className="flex-grow overflow-y-auto space-y-4 mb-20 pr-2 no-scrollbar">
        ...
        <div className="fixed inset-0 pointer-events-none flex items-center justify-center opacity-5 z-0 select-none overflow-hidden">
          <div className="transform -rotate-12 text-9xl font-black text-white whitespace-nowrap">
            555+ LOL 555+
          </div>
        </div>

        {/* Marquee Warning */}
        <div className="w-full bg-yellow-900/50 text-yellow-200 text-[10px] py-1 px-2 rounded overflow-hidden whitespace-nowrap mb-2 border border-yellow-700/50">
        ...
        {messages.length === 0 && (
          <div className="text-center text-gray-500 mt-10 italic relative z-10">
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex flex-col relative z-10 ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
            <div 
              className={`max-w-[85%] p-3 rounded-2xl text-sm transition-transform hover:scale-[1.02] ${
                msg.role === 'user' 
                  ? 'bg-kuba-yellow text-black rounded-tr-none shadow-[4px_4px_0px_0px_rgba(255,255,255,0.2)]' 
                  : 'bg-white text-black rounded-tl-none border-2 border-gray-200 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.5)] rotate-1'
              }`}
            >
            ...
            {msg.role === 'model' && (
              <div className="flex gap-2 mt-1 ml-1 transform -rotate-1">
                <button 
                  onClick={() => handleFeedback(msg.id, 'up')}
                  className={`p-1 rounded-full border border-gray-700 bg-gray-900 transition-all active:scale-90 ${msg.feedback === 'up' ? 'text-green-400 border-green-500' : 'text-gray-500 hover:text-green-400'}`}
                ...
                <button 
                  ...
                  className={`p-1 rounded-full border border-gray-700 bg-gray-900 transition-all active:scale-90 ${msg.feedback === 'down' ? 'text-red-400 border-red-500' : 'text-gray-500 hover:text-red-400'}`}
                  disabled={!!msg.feedback}
                >
                  👎
                </button>
        ...
        {isLoading && (
          <div className="flex justify-start relative z-10">
             <div className="bg-white text-black p-3 rounded-2xl rounded-tl-none text-xs font-bold animate-bounce shadow-lg rotate-1">
               Thinking of a roasted joke... 🤔
             </div>
          </div>
        )}
      ...
      <div className="fixed bottom-20 left-4 right-4 max-w-md mx-auto z-20">
        <div className="flex gap-2">
          <input 
            ...
            className="flex-grow bg-gray-900/90 backdrop-blur-sm text-white border-2 border-gray-600 rounded-xl px-4 py-3 focus:outline-none focus:border-kuba-yellow focus:ring-2 focus:ring-kuba-yellow/50 placeholder-gray-500 transition-all"
          />
          <button 
            onClick={handleSendMessage}
            disabled={state.dailyQuota <= 0 || isLoading}
            className={`px-4 rounded-xl font-bold transition-all text-xl ${
              state.dailyQuota > 0 && !isLoading
              ? 'bg-kuba-yellow text-black border-2 border-black shadow-[4px_4px_0px_0px_#fff] active:translate-x-1 active:translate-y-1 active:shadow-none hover:-translate-y-1'
